...
**Status (21 September 2026): M8b S5 implemented** - see `docs/reviews/milestone-8-s5.md`. S5 is approval-only and performs zero browser writes: migration `0010` adds `protected_values` (eight closed kinds, plaintext local task data, masked previews, SHA-256 digests) and a `form_prepare` task-grant kind; a separately confirmed form-planning grant lets one provider see bounded form structure and masked previews; a `prepare_form` proposal is validated and turned into a digest-bound `DisclosureManifest`; a trusted exact approval is spent and ends in `prepared_nothing`. **Roadmap:** M8b - S4 ✅; **S5 ✅ disclosure manifest / exact approval**; **S6 next** (frozen local form draft). M8b is not complete; S6, M9 and M10 have not started.

**Status (21 September 2026): M8b S6 implemented** - see `docs/reviews/milestone-8-s6.md`. S6 fills an authenticated form in Lumi's own headed browser with the network frozen at two layers, verifies each value from the live DOM, keeps the page frozen, and hands it over only on a second exact approval (or destroys it while frozen on discard). Migration `0011`. **M8b engineering implementation is complete. The real-account release gate is BLOCKED (Authenticode `NotSigned`) and real-form shipping validation is PENDING**: the three-unadapted-real-forms criterion of the conditional-go was not and must not be attempted. M9 and M10 have not been started.
