...
**Local and private.** Every observation is `desktop_private` and `untrusted_environment`. In S1 it goes to **no** provider, voice model, planner, memory, research context or task summary. A planted marker in a fixture edit control is present in `desktop_observations` and in no other table, log or output, and the TypeScript side has no reference to the routes, the storage or the schema (`tests/test_desktop_source.py`, `src/main/agent/desktop-firewall.test.ts`). S2 changed those tests on purpose: exactly one reviewed path (below) may read an observation, and only after an exact trusted approval. Diagnostics carry counts, a truncation flag, a duration, a worker generation, an observation id and an error code, never a title, a name, text, a PID/HWND, a path or a control ref.
...
## Desktop disclosure and read-only reasoning (Milestone 9 S2)

S2 changes **who may see an already-captured snapshot**, and nothing else. The desktop worker is unchanged and still observation-only: S2 adds no OS call, no input, no focus, no scroll, no screenshot, no vision and no coordinate, and the S1 zero-input scanner and exact allowlists pass unchanged.

**Exact observation, exact approval.** A `desktop_disclose` grant (a `task_grants` kind, not a second framework) binds one task, one observation id and that observation's snapshot digest, one surface identity and worker generation, one recipient **and model**, one redaction policy, one provider call and a ten-minute expiry. Observing the window again produces a new observation and digest, so a new approval is required: approving one snapshot is not approval for whatever the same window shows next. There is no continuous watching and no model loop. A disclosure grant is **not** a control grant, and none exists in S2.

**Trusted path only.** The question is typed locally and never passes through the conversation model. The renderer supplies a surface identity and a question; **main** chooses the provider from its own configuration (the renderer cannot). The card is built by the runtime; the renderer's approval names only a grant id and the revision it showed. Voice cannot select, confirm or reject: the desktop controller is not a member of the voice backend, and there is no spoken or typed-text route to the grant. Six exact IPC methods exist; none takes a method name, route, provider, snapshot, handle or coordinate, and none can focus, invoke, type, select, scroll, click or launch.

**Once, durably, before the provider.** The single-use grant is swapped `ACTIVE -> COMPLETED` (compare-and-swap on revision, status and expiry) and a disclosure row is inserted (`grant_id` and `task_id` UNIQUE) in **one transaction that commits before** the projection is returned to main. No database transaction is open while the provider is called. If the runtime dies, the result is lost, or the claim is never reported within five minutes, the disclosure becomes `OUTCOME_UNKNOWN`: Lumi cannot know whether the provider received the snapshot, so it is never replayed and never treated as an ordinary failure. Revoking after a claim cannot un-send it, and the record does not pretend otherwise. A retry needs a fresh observation and a fresh trusted approval.

**One attempt.** The router class `desktop_planning` is private: the request must carry the approval's one-recipient rule (`permits` also pins the model), there is no failover, no image, and a failure, timeout, unparseable reply or ungrounded answer ends the run. Provider availability is checked before the claim, so an unavailable provider spends nothing.

**What the provider sees.** A second, provider-specific projection of that one observation: closed semantic fields only (`controlRef`, `parentRef`, role, name, text, enabled, visible, focused, selected, checked, expanded), at most 120 nodes and 8 KB of text, plus the capture time and truncation flags. Every name and text goes through the identifier redactor (emails, phone-shaped values, Luhn-valid card numbers, nine-or-more-digit runs) **before** it is counted, digested or returned. The window title and application label are card-only display text: the top-level window node keeps its role but not its name, and any node whose name or text equals a withheld display string is emptied. No handle, process, path, AutomationId, class, RuntimeId, coordinate, pattern list, other window, other observation, conversation, memory or browser context is sent. Application text is delimited as untrusted data ("data copied from desktop application").

**Read-only output with located quotes.** The result is `{schemaVersion:1, kind:"answer", answer, evidence:[{controlRef, quote}]}` or `{kind:"cannot_answer", reason}` from a closed set. Any other key (operation, tool, action, focus, invoke, click, coordinates, approval, grant, provider) refuses the whole reply, in TypeScript and again in the runtime. The runtime recomputes the projection from the persisted observation and requires every `controlRef` to exist in it, every quote to occur in that control's *redacted* name or text (a raw identifier can never verify; a quote shorter than six characters only counts when it is the control's whole text), and every number in the answer to occur in a quote (`answer_not_grounded`). The evidence that is stored is the projection's own text, not the provider's string. This shows the quotes are in the approved snapshot, not that the answer reads them correctly. The answer is stored `desktop_private` in `desktop_answers`; it is never memory, conversation context or another task's context.

**Diagnostics and events** carry ids, digests, counts, statuses and closed codes only: never desktop text, a title, the question, a quote or an answer.

**Residual risks (honest).**
- The provider sees private application text after explicit approval, and its answer may repeat it.
- Redaction is pattern-based and is not anonymisation: names, addresses, short numbers, usernames and anything that reads as prose remain; an identifier cut by the 120-character node limit may be unmatched; a title quoted *inside* other text is not withheld.
- Grounding proves that a quote is in the approved snapshot, not that the model understood it.
- The snapshot is a photograph: it can be stale the moment after capture, UI Automation may omit content, and truncation means the provider may not see the whole application (the card and the prompt say so).
- Application text remains prompt-injection-capable untrusted data; the only defence in depth is that S2 has no action to inject into.
- The window title and application label (display only, in the immutable grant scope), the user's question and the redacted answer are stored durably; only the raw observation is retained under S1's 25-row / 24-hour rule.
- S1 clips text at 120 characters before redaction, so an identifier can be cut in half; the projection drops a trailing partial address or long digit run at a clipped edge, best effort.
- No desktop action exists yet. The missing production Authenticode certificate remains an unrelated release blocker.

## Desktop focus, semantic scroll and registered launch (Milestone 9 S3)

S3 is the first Windows state-changing slice. It adds **exactly three effects**, each behind an exact, single-use, per-action trusted approval, and nothing else as of S3: no Invoke, SetValue, selection, mouse, keyboard, hotkey, drag, clipboard, screenshot, coordinate, shell, or arbitrary executable, path or argument. Desktop disclosure (S2) is not authority for any of them. (S4, below, adds exactly three more: Invoke, SetValue and selection. S5, further below, adds exactly one scoped screenshot capability, behind its own approvals. Mouse, keyboard, hotkey, drag, clipboard, coordinate and shell remain absent through S5 too, and there is still no generic screenshot route: capture only ever targets one already-approved window's client area.)
...
**Focus** is UIA `SetFocus` on the window root of an already-visible ordinary window (measured: the Windows foreground lock refuses a raw `SetForegroundWindow` from a background process, so no Win32 foreground call, `AttachThreadInput`, `ShowWindow` or synthesized input exists). Minimized/hidden windows are refused, never restored. **Scroll** is UIA `ScrollPattern.Scroll` by a closed step on a control re-derived from the live tree; the old observation's refs die before the effect and a fresh observation follows. **Launch** names a registered application by id only; the descriptor (canonical path, fixed args) is trusted configuration, refused unless it is a local `.exe` under an administrator-writable install root (not a shell, interpreter, LOLBin, credential UI, user-writable directory or Lumi itself); the process is started as an argument list with no shell, an allowlisted environment (no Lumi credential) and its own directory, outside the runtime's kill-on-close job.

**Before every effect** the worker re-proves the worker generation, the surface `(ref, epoch, pid, hwnd, creation time)`, that it is not Lumi's own tree, not elevated or credential UI, holds no credential input, and that the person has not used the keyboard or mouse since the approval (`GetLastInputInfo`: an opaque tick, never input, no hook). After an effect, any doubt is `OUTCOME_UNKNOWN`, never a known failure; a lost, late or misaddressed answer is unknown and is never retried; a runtime that dies mid-call is recovered as unknown; a duplicate launch is prevented by inspecting for a running instance of the exact registered executable (and by adopting an unverified earlier spawn) before any spawn.

**Source-level enforcement.** The S1 scanner became an exact-capability scanner: `SetFocus`, `Scroll` and the single `Popen` are pinned by call site and by AST shape; `effects_win32.py` may call only five query/handle functions; the job breakaway flag may appear in two modules. The independent review found and closed four scanner holes.

**Residual risks (honest).** The runtime's job now allows a process that asks to break away (only the launch module asks; not verified for Chromium/libuv). A registered executable replaced by an administrator between validation and launch defeats the path check. Path-only instance identity cannot deduplicate launcher-stub applications (register only applications whose process is the registered executable). A key-up after the click can cause a spurious takeover refusal (fails safe). Window titles and control names are stored in the immutable proposal for the card (not in dispatch rows, events, logs or generic reads). No production-signed installed validation exists. **Found by the final cross-slice audit, deliberately not narrowed:** `focus`'s human-input check runs once, before a fresh `root_for` COM round-trip and the native `SetFocus` call both — wider than every other S3/S4/S5 effect's own second check (whose element/pattern is already held before their final check runs, so only the native call's own pattern acquisition remains unchecked). Splitting the call to hold the root across an added check would require the source scanner's `focus-target` pin — `.focus()` may be called only in the exact shape `root_for(...).focus()`, precisely so a held reference can never dodge it — to be loosened to recognise a safe local-variable alias; that scanner change was judged riskier to make under audit time pressure than the gap itself, for a Medium-severity, already-bounded residual. See `docs/reviews/milestone-9-final.md`.

## Desktop bounded semantic actions (Milestone 9 S4)

S4 adds exactly three more effects to S3's desktop action ledger: `ValuePattern.SetValue`, `SelectionItem.Select` and `InvokePattern.Invoke` (currently one reviewed effect only, `NAME_TOGGLE`: pressing a control expected to change its own accessible name, matching a short, reviewed allowlist of non-destructive disclosure/expand-collapse labels). Still no mouse, keyboard, hotkey, drag, clipboard, shell, coordinate or generic Invoke/SetValue/Select route.

**Two authorities, never one.** A model may *propose*, never authorize: exactly one of `invoke(controlRef)` / `set_value(controlRef, valueRef)` / `select(containerRef, optionRef)`, from one exact, redacted disclosure of one observation, through a private planning task class (`desktop_action_planning`, one recipient, zero failover, no image) — a structural twin of S2's read-only disclosure, recorded in its own `desktop_action_plans` row. The model never sees a raw value, a coordinate, a native id, a provider or an approval; it names only opaque refs, and for a value, an opaque local ref the runtime alone resolves to trusted text. `DesktopActionService.propose_from_plan` then independently re-verifies the whole plan against a freshly rebuilt projection — never trusting the plan's own recorded output — and opens an ordinary `DESKTOP_SET_VALUE`/`DESKTOP_SELECT`/`DESKTOP_INVOKE` action on the same S3 ledger, behind its own SECOND, separate, exact execution approval. Disclosure authority is never execution authority.

**Before every mutation**, in order: refuse a repeated dispatch id and replay a finished one; re-prove the surface and re-derive the control by exact live match (which re-runs Lumi/elevation exclusion along the target's own ancestor path); a fresh, whole-surface credential re-scan (not just the target's ancestor path, closing a gap the S4 review found); a second, S4-only check refusing a target whose *process* is a shell, script host, credential/consent broker or the Windows security app, independent of role or label; refuse if the human has touched the machine since the approval baseline; kill the old observation's refs; refuse a second time on human input immediately before the one native call; perform it; verify from **fresh** evidence, never the call's own return value. A verifying re-read that itself fails (not "no effect", but "could not tell") is `OUTCOME_UNKNOWN`, never a known failure — the write may have happened.

**Invoke is allowlisted, not denylisted.** The worker's own structural verifier (`NAME_TOGGLE`) is what actually proves an effect — a name-change re-derivation that matches every ancestor step as before but the target itself by identity, not name, because the name is expected to have changed. On top of that, an *additional*, attacker/application-controlled but fail-closed layer requires the control's exact current accessible name to match a short, reviewed, non-destructive allowlist (`show details`, `expand`, …); an unrecognized label refuses rather than defaults to allowed. A real submit/payment/delete button, however labelled, is never reachable this way: only the allowlisted labels reach the Invoke call at all, and only a genuine name-change is ever reported as done.

**The unresolved-mutation lock.** A `SetValue`/`Select`/`Invoke` left `OUTCOME_UNKNOWN` or `RECONCILING` blocks every other desktop action — of any operation, in any task — until a person reports `succeeded` / `failed` / `still_unknown` through a dedicated reconciliation step; nothing re-derives or retries the effect automatically, and the block is checked at every point a new plan, confirmation, claim or execution proposal could otherwise start, not only at the execution ledger's own entry point.

**The raw value, end to end.** The person's typed text is stored durably in exactly one place: the plan's immutable `task_grants.scope` (`StoredValue.raw`, never re-editable after insert, never copied into the plan's own recorded output, a dispatch row, a task event or a log). It is never in the provider payload (the model sees only a ref, a person-chosen label and a length); the durable action-ledger row (`actions.proposal`) carries only the opaque `value_ref`, and every read of a pending or finished set-value action — the initial card, a later status poll, and the approval call that performs the write — re-resolves the exact text fresh from that one immutable scope rather than trusting a second, independently-stored copy. The generic (non-desktop) action-read route never returns it under any name. The dedicated, Electron-main-only desktop action route is the one place it is shown, because the trusted card needs to.

**Residual risks (honest).** The safe-Invoke label allowlist is English-centric and label-based, exactly like S1's credential-name heuristic and S2's title-withholding heuristic — additional defence, never the thing that makes Invoke safe by itself. File-picker detection is a best-effort window-title match, not a structural signal. The human-input-takeover check before a native mutation call cannot be closed to exactly zero: a second check runs immediately before `SetValue`/`Select`/`Invoke`, narrowing the gap to that one call's own unavoidable COM pattern acquisition. Reconciliation is a human report, not an automated read-verified check: `still_unknown` is always safe, but a person could in principle misreport `succeeded`. `InvokePattern`'s success path is thoroughly proven against fakes; proving it end to end against real UIA needs a fixture control that renames itself on press, not added as of S4/S5 — a documented, honest PENDING, not a claimed pass. No production-signed installed validation exists.

## Desktop scoped visual fallback (Milestone 9 S5)

S5 adds exactly one more capability, `capture`, and it is read-only: no OS mutation, no `OUTCOME_UNKNOWN` story the way a write has one. It exists to answer one question UIA sometimes cannot: what does this window actually look like. It never adds a way to act on what it sees.

**The fallback trigger is deterministic, local code, never a model's preference.** `classify_fallback_eligibility` (pure function, `app.domain.desktop_vision`) inspects an already-fresh UIA observation and returns one of exactly three closed reasons (`uia_empty`, `uia_missing_required_semantics`, `uia_truncated_without_target`) or `None`. A capture card is offered only when this function returns a reason; nothing else can open one.

**Two separate authorities, not one.** Capturing pixels (`kind = desktop_vision_capture`) authorises exactly one screenshot, for LOCAL use only (on-device display, best-effort local OCR) -- no provider is ever contacted for this grant kind, and the immutable scope names no recipient because there isn't one. Sending an image to a provider (`kind = desktop_vision_disclose`) is a SEPARATE grant, created only after a capture SUCCEEDED, naming one provider, one model and the person's own typed purpose. Approving one never authorises the other: text disclosure authority from S2/S4 does not extend to images either, and a `desktop_vision_disclose` grant's own `claim` step always takes a BRAND NEW screenshot -- never the first capture's own bytes, so "a fresh image needs fresh approval" is a structural fact (a new `capture_id` and a new worker call every time), not a policy note.

**The worker's one native call.** `capture()` (`app/desktop/effects.py`) follows the same order S3/S4 already established: re-prove the surface via `SurfaceTable.resolve` (worker generation, `(ref, epoch)`, not Lumi, not elevated, not a credential broker) → a fresh whole-surface credential re-scan (the same one S4's mutations added) → refuse if the person touched the machine since the approval baseline → a second, narrower refusal immediately before the ONE native call (`PrintWindow`, `app/desktop/capture_win32.py`, exactly one call site, exact-shape pinned) → verify the surface is still the same afterward. Distinct from S4: `capture_scope_uncertain` (`app.desktop.dpi.capture_scope_certain`) refuses BEFORE the native call if the client-area crop cannot be proven exact, rather than guessing at a rectangle.

**Frame identity.** `geometry_fingerprint` (`app.desktop.dpi`) is a value-free digest of window rect + client rect + monitor + DPI + process creation identity; it changes on ANY material change (move, resize, monitor change, DPI change, or the window being replaced by a different process instance) and is computed fresh on every capture -- there is no "reuse an old frame" code path anywhere, because every claim performs its own new native call. The worker declares `PER_MONITOR_AWARE_V2` DPI awareness once, at construction, so `GetWindowRect`/`GetClientRect`/`ClientToScreen` return true physical pixels rather than DPI-virtualized ones; monitor matching (`monitor_for_rect`) is pure, tested code, not trusted to `MonitorFromWindow`'s own opaque behaviour.

**What crosses which boundary.** The worker encodes the captured pixels to PNG itself (`app/desktop/png_encode.py`, a small stdlib-only `zlib` encoder -- no Pillow, no `mss`, no screenshot library anywhere in this codebase, matching the project's dependency policy) before anything leaves `app/desktop/`. The Python service persists only digests, dimensions, DPI and monitor id (`desktop_captures`, `desktop_vision_disclosures`); the base64 image itself is never written to Postgres, a log line, a task event payload or the generic (non-desktop) action-read route. It exists in memory for exactly one hop each: worker → Python service → Electron main (local use), and worker → Python service → Electron main → the one approved provider (disclosure).

**The router's one narrow exception.** `PRIVATE_VISION_TASK_CLASSES` (`src/main/models/model-router.ts`) is a new, disjoint allowlist naming exactly one class, `desktop_vision`; it is the only task class in the whole router permitted to carry `request.image` while private. Every other private class (`authenticated_planning`, `authenticated_answer`, `form_planning`, `desktop_planning`, `desktop_action_planning`) still hard-refuses an image, unchanged. `desktop_vision` still gets the same "one recipient, zero failover" structural guarantee every private class gets for free.

**A vision result is evidence only.** `VisionResult`/`VisionCandidate` (Python, `extra="forbid"`) and `parseDesktopVisionResult`/`AgentVisionCandidate` (TypeScript, exact-key parsing) have no field for a click, a key, a coordinate, an approval or an action anywhere in the schema; a region is normalised to the captured crop (0..1), never a screen or window coordinate. Nothing in this codebase — not the renderer panel, not the controller, not the action ledger — turns a candidate into anything that can run. If semantic UIA cannot re-establish an actionable target after a screenshot, that is `manual_handoff_required` territory, not a coordinate click; this codebase still has no coordinate-click primitive anywhere, S5 included.

**Source-level enforcement.** The S1/S3/S4 scanner gained a third reviewed native-surface file, `capture_win32.py`, pinned by its own exact allowlist (`PINNED_CAPTURE_WIN32`) exactly like `win32.py` and `uia_backend.py`; `PrintWindow` is pinned to one call site inside `capture()`, the same AST-shape discipline `SetValue`/`Select`/`Invoke` already have. `bitblt`, `getdc` and a blind full-screen `screenshot`/`grab` remain forbidden everywhere, including inside `capture_win32.py` itself.
